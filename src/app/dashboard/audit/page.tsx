...
//     // AUDIT-SPECIFIC COLOR THEME - Distinct from main app colors
//     const getStatusColor = useCallback((status: string): string => {
//         const auditStatusColors: { [key: string]: string } = {
//             // Primary audit statuses - using distinct colors
//             'scanning': 'audit-blue',
//             'rma': 'audit-orange',
//             'resolved': 'audit-green',
//             'chargeback': 'audit-red',
//             'active': 'audit-teal',
//             'sold': 'audit-purple',
//             'demo': 'audit-indigo',

//             // Secondary statuses
//             'adjustment': 'audit-amber',
//             'returned': 'audit-cyan',
//             'damaged': 'audit-rose',
//             'lost': 'audit-gray',
//             'in stock': 'audit-emerald',
//             'shipped': 'audit-violet',

//             // Process statuses
//             'pending': 'audit-yellow',
//             'completed': 'audit-lime',
//             'cancelled': 'audit-pink',
//             'refunded': 'audit-fuchsia',
//             'in progress': 'audit-sky',
//             'waiting for parts': 'audit-amber',

//             // Technical statuses
//             'sold (glitch)': 'audit-warning',
//             'repaired': 'audit-success',
//             'overage': 'audit-info',
//             'audit': 'audit-primary',
//             'verified': 'audit-secondary'
//         };

//         const normalizedStatus = status.toLowerCase().trim();
//         return auditStatusColors[normalizedStatus] || 'audit-default';
//     }, []);
...
//                                     })}>
//                                         <td>
//                                             <span className={`audit-status-indicator audit-status-${status.color}`}>
//                                                 {status.status}
//                                             </span>
//                                         </td>
//                                         <td className="audit-col-right">{status.count}</td>
...
//                                         <td className="audit-col-right">{cost}</td>
//                                         <td>
//                                             <span className={`audit-status-indicator audit-status-${getStatusColor(String(status))}`}>
//                                                 {status}
//                                             </span>
//                                         </td>
//                                         <td>{comments}</td>
...
export default function AuditDashboard() {
    ...
    // Enhanced detailed data filtering with product/SKU filters for audit
    const filteredDetailedData = useMemo(() => {
        let filtered = currentData; // Use currentData for detailed view

        // Apply product filter - search in SKU Description
        if (productFilter.trim()) {
            const productLower = productFilter.toLowerCase().trim();
            filtered = filtered.filter((report) =>
                getField(report, ["IMEI", "FullIMEI#", "Serial"])?.toLowerCase().includes(productLower)
            );
        }
        ...
        if (skuFilter.trim()) {
            ...
            filtered = filtered.filter((report) =>
                getField(report, ["SKUDescription", "SKU", "sku"])?.toLowerCase().includes(skuLower)
            );
        }

        return filtered;
    }, [currentData, productFilter, skuFilter, getField]);

    // Apply pagination to detailed data
    const totalPages = Math.ceil(filteredDetailedData.length / itemsPerPage);
    const paginatedData = useMemo(() => {
        const startIndex = (currentPage - 1) * itemsPerPage;
    ...
    const renderDetailedTable = useCallback((data: AuditItem[]) => {
        // For detailed view, we use the filteredDetailedData which includes product/SKU filters
        const detailedData = filteredDetailedData;
        const totalRecords = detailedData.length;
        ...
        return (
            <div className="audit-table-block">
                ...
                {/* ADD FILTER CONTROLS FOR AUDIT DETAILED VIEW */}
                <div className="audit-filter-controls">
                    <div className="filter-group">
                        <label htmlFor="product-filter">Filter by IMEI</label>
                        <input
                            id="product-filter"
                            type="number"
                            placeholder="354123756xxxxxx"
                            value={productFilter}
                    ...
                    <div className="filter-group">
                        ...
                        <input
                            id="sku-filter"
                            type="text"
                            placeholder="e.g., XT24191, XT25131..."
                            value={skuFilter}
                ...
                <div className="audit-table-wrapper">
                                    ...
                                    <tr key={index}>
                                        ...
                                        <td>
                                            <div className="sku-info">
                                                <div className="sku-description">{sku.split('|').pop() || sku}</div>
                                            </div>
                                        </td>
                                        <td>{imei}</td>
                                        <td className="audit-col-right">{cost}</td>
                    ...
                    {paginatedDetailedData.length === 0 && (
                        <div className="no-data">
                            {searchTerm || productFilter || skuFilter
    ...
    }, [selectedStatus, getField, formatCurrency, parseCurrency, detectedFields, getStatusColor, searchTerm, currentPage, itemsPerPage, productFilter, skuFilter, filteredDetailedData]);
