...
export default function IASReportsPage() {
    ...
    const filteredDetailedData = useMemo(() => {
        ...
        if (productFilter.trim()) {
            ...
            filtered = filtered.filter((report) =>
                report.Product?.toLowerCase().includes(productLower) ||
                report.Model?.toLowerCase().includes(productLower) ||
                (report.Product + " " + report.Model)?.toLowerCase().includes(productLower)
            );
        }
        ...
        if (skuFilter.trim()) {
            ...
            filtered = filtered.filter((report) =>
                report.SKU?.toLowerCase().includes(skuLower)
            );
        }

        return filtered;
    }, [filteredData, productFilter, skuFilter]);
    ...
    const handleExportCSV = () => {
        const csvData = currentView === "detailed" ? filteredDetailedData : currentData;
        if (csvData.length) {
            const keys = Object.keys(csvData[0]);
            const csv = [keys.join(",")]
                .concat(
        ...
        return (
            <div className="ias-table-block">
                <div className="ias-table-header">
                    ...
                    <div className="ias-meta">
                        {filteredDetailedData.length} inventory records
                        {searchTerm && ` matching "${searchTerm}"`}
                        {(productFilter || skuFilter) && (
                ...
                <div className="ias-filter-controls">
                    <div className="filter-group">
                        <label htmlFor="product-filter">Filter by Product/Model:</label>
                        <input
                            id="product-filter"
                            type="text"
                            placeholder="e.g., iPhone 16, MOT XT24191, Samsung, G STYLUS..."
                            value={productFilter}
                                    ...
                                    <React.Fragment key={uniqueId}>
                                        <tr className="main-row">
                                            ...
                                            <td>
                                                <div className="product-info">
                                                    <div className="product-name">{report.Product}</div>
                                                    <div className="product-model">{report.Model}</div>
                                                </div>
                                            </td>
                                            <td>
                                                <div className="sku-info">
                                                    <div className="sku-code">{report.SKU}</div>
                                                </div>
                                            </td>
                                            <td className={`ias-col-right ${parseIntSafe(report.INSTOCK) > 0 ? "in-stock" : "out-of-stock"}`}>
                                                {report.INSTOCK}
