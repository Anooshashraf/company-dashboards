...
//     const [isLoadingData, setIsLoadingData] = useState<boolean>(true);
//     const [currentView, setCurrentView] = useState<
//         "regions" | "market" | "store" | "status" | "detailed"
//     >("regions");
...
//     const detectKey = (candidates: string[], data: IASItem[]): string => {
//         if (!data || data.length === 0) return candidates[0];
...
//     const detectAllFields = (data: IASItem[]) => {
//         const fields = {
//             market: detectKey(["Market", "MARKET"], data),
//             store: detectKey(["Store Name", "Store", "STORE"], data),
//             status: detectKey(["Status", "STATUS"], data),
//             cost: detectKey(["Cost", "COST", "TOTAL COST"], data),
//             netWorth: detectKey(["Net Worth", "Net Worth"], data),
//             sku: detectKey(["SKU", "sku"], data),
//             model: detectKey(["Model", "MODEL"], data),
//             product: detectKey(["Product", "PRODUCT"], data),
//             instock: detectKey(["INSTOCK", "In Stock", "IN STOCK"], data),
//             total: detectKey(["TOTAL", "Total", "total"], data),
//             allocation: detectKey(["ALLOCATION", "Allocation"], data),
//             address: detectKey(["Store Address", "Address", "ADDRESS"], data),
//             subMarket: detectKey(["Sub Market", "SubMarket", "SUB MARKET"], data)
//         };
...
//     // Create regions based on markets since your IAS data doesn't have regions
//     const createRegionsFromMarkets = (data: IASItem[]) => {
//         const markets = Array.from(new Set(data.map(row => getField(row, ["Market", "MARKET"]))));

//         // Group markets into regions - this will be dynamic based on your data
//         const regionMapping: { [key: string]: string[] } = {
//             "Aleem Ghori Region": [],
//             "Hasnain Mustaqeem Region": []
//         };

//         // Distribute markets between the two regions
//         markets.forEach((market, index) => {
//             if (index % 2 === 0) {
//                 regionMapping["Aleem Ghori Region"].push(market);
//             } else {
//                 regionMapping["Hasnain Mustaqeem Region"].push(market);
//             }
//         });
...
//             // Find which region this market belongs to
//             let regionName = "Other Regions";
//             for (const [region, marketList] of Object.entries(regionMapping)) {
...
//                 regions[regionName] = [];
//             }
//             regions[regionName].push(row);
//         });

//         return regions;
//     };

//     const aggregate = (data: IASItem[], keyField: string, level: string = ""): AggregatedGroup[] => {
//         const groups: {
//             [key: string]: {
//                 count: number;
//                 instock: number;
...
//             if (level === "regions") {
//                 // For regions level, we create custom regions
//                 const market = getField(row, ["Market", "MARKET"]);
//                 const regions = createRegionsFromMarkets([row]); // Get region for this row
//                 const regionName = Object.keys(regions).find(region =>
//                     regions[region].some(r => getField(r, ["Market", "MARKET"]) === market)
//                 ) || "Other Regions";
//                 key = regionName;
//             } else {
//                 keyRaw = row[keyField];
...
//             groups[key].count += 1;
//             groups[key].instock += parseNumber(row[detectedFields.instock]);
//             groups[key].total += parseNumber(row[detectedFields.total]);
//             groups[key].cost += parseCurrency(row[detectedFields.cost]);
//             groups[key].rows.push(row);
//         });
...
//     const buildStatusSummary = (data: IASItem[]): StatusSummary[] => {
//         const statusGroups: { [key: string]: StatusSummary } = {};
//         const statusField = detectedFields.status;

//         data.forEach((row) => {
//             const rawStatus = String(row[statusField] || "").trim();
//             const cost = parseCurrency(row[detectedFields.cost]);
//             const instock = parseNumber(row[detectedFields.instock]);
//             const total = parseNumber(row[detectedFields.total]);
...
//             statusGroups[cleanStatus].count += 1;
//             statusGroups[cleanStatus].instock += instock;
//             statusGroups[cleanStatus].total += total;
//             statusGroups[cleanStatus].cost += cost;
//         });
...
//         const totalInstock = data.reduce((s, r) => s + parseNumber(r[detectedFields.instock]), 0);
//         const totalInventory = data.reduce((s, r) => s + parseNumber(r[detectedFields.total]), 0);
//         const totalCost = data.reduce((s, r) => s + parseCurrency(r[detectedFields.cost]), 0);
...
//     // Navigation handlers for the hierarchy - EXACTLY like audit dashboard
//     const handleRegionClick = (region: AggregatedGroup) => {
//         const marketData = region.rows;
//         setCurrentData(marketData);
//         setCurrentView("market");
...
//             { level: "Regions" },
//             { level: "Market", selected: selectedRegion },
//             { level: "Store", selected: market.key },
//         ]);
//     };

//     const handleStoreClick = (store: AggregatedGroup) => {
//         const statusData = store.rows;
//         setCurrentData(statusData);
//         setCurrentView("status");
...
//             { level: "Regions" },
//             { level: "Market", selected: selectedRegion },
//             { level: "Store", selected: selectedMarket },
//             { level: "Status", selected: store.key },
//         ]);
//     };
...
//             { level: "Market", selected: selectedRegion },
//             { level: "Store", selected: selectedMarket },
//             { level: "Status", selected: selectedStore },
//             { level: "Detailed", selected: status.key },
//         ]);
//     };
...
//                 setSelectedRegion("");
//             } else if (previousLevel.level === "Market") {
//                 // For market level, we need to filter by custom region logic
//                 const regions = createRegionsFromMarkets(filteredData);
//                 const marketData = regions[previousLevel.selected!] || [];
//                 setCurrentData(marketData);
//                 setCurrentView("market");
//                 setSelectedMarket("");
//             } else if (previousLevel.level === "Store") {
//                 const marketKey = detectedFields.market;
//                 const storeData = filteredData.filter(
//                     (row) => getField(row, [marketKey]) === previousLevel.selected
//                 );
//                 setCurrentData(storeData);
//                 setCurrentView("store");
//                 setSelectedStore("");
//             } else if (previousLevel.level === "Status") {
//                 const marketKey = detectedFields.market;
//                 const storeKey = detectedFields.store;
//                 const statusData = filteredData.filter(
//                     (row) =>
//                         getField(row, [marketKey]) === selectedMarket &&
//                         getField(row, [storeKey]) === previousLevel.selected
//                 );
//                 setCurrentData(statusData);
//                 setCurrentView("status");
//                 setSelectedStatus("");
...
//     const renderStatusSummary = (data: IASItem[]) => {
//         const statusSummary = buildStatusSummary(data);
//         const totalCost = statusSummary.reduce((sum, item) => sum + item.cost, 0);
...
//             <div className="audit-table-block">
//                 <div className="audit-table-header">
//                     <h2>Status Summary - {selectedStore}</h2>
//                     <div className="audit-meta">
//                         {statusSummary.length} status types — total value {formatCurrency(totalCost)}
//                     </div>
//                 </div>
...
//                             <tr>
//                                 <th>Status</th>
//                                 <th className="audit-col-right">Store Count</th>
//                                 <th className="audit-col-right">IN STOCK</th>
//                                 <th className="audit-col-right">Total Inventory</th>
//                                 <th className="audit-col-right">Total Value</th>
//                                 <th>Value Distribution</th>
//                             </tr>
//                         </thead>
...
//                                         </td>
//                                         <td className="audit-col-right">{status.count}</td>
//                                         <td className="audit-col-right">{status.instock}</td>
//                                         <td className="audit-col-right">{status.total}</td>
//                                         <td className="audit-col-right">{formatCurrency(status.cost)}</td>
//                                         <td>
//                                             <div className="audit-bar-cell">
//                                                 <div className="audit-bar-track">
...
//         switch (level) {
//             case "regions":
//                 keyField = "market"; // We'll create custom regions from markets
//                 title = "Regions";
//                 break;
//             case "market":
//                 keyField = detectedFields.market;
...
//                                         <td>{group.key}</td>
//                                         <td className="audit-col-right">{group.count}</td>
//                                         <td className="audit-col-right">{group.instock}</td>
//                                         <td className="audit-col-right">{group.total}</td>
//                                         <td className="audit-col-right">{formatCurrency(group.cost)}</td>
//                                         <td>
//                                             <div className="audit-bar-cell">
//                                                 <div className="audit-bar-track">
...
//             <div className="audit-table-block">
//                 <div className="audit-table-header">
//                     <h2>Detailed Report - {selectedStatus}</h2>
//                     <div className="audit-meta">{data.length} inventory records</div>
//                 </div>
...
//                                 <th>Product</th>
//                                 <th className="audit-col-right">IN STOCK</th>
//                                 <th className="audit-col-right">TOTAL</th>
//                                 <th className="audit-col-right">Cost</th>
//                                 <th className="audit-col-right">Net Worth</th>
//                                 <th>Address</th>
//                             </tr>
//                         </thead>
//                         <tbody>
...
//                                 const instock = parseNumber(row[detectedFields.instock]);
//                                 const total = parseNumber(row[detectedFields.total]);
//                                 const cost = formatCurrency(parseCurrency(row[detectedFields.cost]));
//                                 const netWorth = formatCurrency(parseCurrency(row[detectedFields.netWorth]));
...
//                                         <td>{model}</td>
//                                         <td>{product}</td>
//                                         <td className="audit-col-right">{instock}</td>
//                                         <td className="audit-col-right">{total}</td>
//                                         <td className="audit-col-right">{cost}</td>
//                                         <td className="audit-col-right">{netWorth}</td>
//                                         <td>{address}</td>
//                                     </tr>
//                                 );
//                             })}
...
//                             renderTable(currentData, "market", handleMarketClick)}
//                         {currentView === "store" &&
//                             renderTable(currentData, "store", handleStoreClick)}
//                         {currentView === "status" &&
//                             renderTable(currentData, "status", handleStatusClick)}
//                         {currentView === "detailed" && renderDetailedTable(currentData)}
//                     </section>
...
export default function IASReportsPage() {
    ...
    const getRegions = (data: IASItem[]) => {
        ...
        // Distribute markets between regions (you can customize this logic)
        markets.forEach((market, index) => {
    ...
    // Navigation handlers - EXACTLY like audit dashboard
    const handleRegionClick = (region: AggregatedGroup) => {
