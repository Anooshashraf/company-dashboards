...
//             const [poData, receivingData, trackingData] = await Promise.all([
...
//         console.log(`🔍 Counting ALL tracking entries for market: ${market}`);
...
//         const count = matchingTrackingRows.length;

//         console.log(`✅ Found ${count} tracking entries for ${market}:`,
//             matchingTrackingRows.map(row => getValue(row, ['Tracking ID', 'ID', 'TrackingID']))
//         );
...
//         // NEW: Get tracking count for this market
...
//             trackingCount // Add tracking count
//         };
//     };
...
//         const regionTotals: { [region: string]: RegionTotals & { trackingCount: number } } = {};
...
//                 groups[regionName] = {
//                     key: regionName,
//                     count: totals.trackingCount, // Use tracking count instead of store count
...
//                 groups[market] = {
//                     key: market,
//                     count: marketTotals.trackingCount, // Use tracking count instead of store count
...
//                 // For store level, we still show 1 as count (each store counts as 1)
//                 groups[storeName] = {
//                     key: storeName,
//                     count: 1,
...
export default function OrderingReportsPage() {
    ...
    const dataSources = [
        ...
        },
        {
            id: 'DISPUTES',
            name: 'DISPUTES',
            gid: '825813925', // Add the correct GID
        ...
        },
        {
            id: 'MAIN',
            name: 'MAIN',
            gid: '2039561125', // Add the correct GID for main sheet
    ...
    const fetchOrderingData = async () => {
        try {
            ...
            // Fetch all data sources
            const [poData, receivingData, trackingData, disputesData, mainData] = await Promise.all([
                fetchDataSource(dataSources[0]),
                fetchDataSource(dataSources[1]),
                fetchDataSource(dataSources[2]),
    ...
    const getMovementsData = (poNumber: string, storeName: string, market: string): string => {
        // Look for movements data in the MAIN sheet data
        const movementRow = mainData.find(row => {
            ...
            // Try to match by store name and market first (since PO number might not be in main sheet)
            return rowStore === storeName && rowMarket === market;
        });
    ...
    const getDisputesData = (poNumber: string, storeName: string, market: string): {
        ...
        // Look for disputes data in disputes sheet - matching by Store Name and Market
        const disputeRow = disputesData.find(row => {
            ...
            console.log('🔍 Checking disputes match:', {
                rowStore,
                storeName,
                rowMarket,
                market,
                match: rowStore === storeName && rowMarket === market
            });
        ...
        if (disputeRow) {
            ...
            console.log('✅ Found disputes data:', { disputeCreated, disputeResolved });
        ...
        console.log('❌ No disputes data found for:', { storeName, market });
        return {
            disputeCreated: '-',
            disputeResolved: '-'
        };
    ...
    const renderDetailedTable = () => {
        // Enhance reports with all data for detailed view
        const enhancedReports = filteredReports.map(report => {
            const poStatus = getPOStatusFromReceiving(report['PO Number'], report['Store Name'], report.Market);
                    ...
                    <table className="ordering-table">
                        <thead>
                            <tr>
                                ...
                                <th className="ordering-col-right">Received Amount</th>
                                <th className="ordering-col-right">Open Amount</th>
                                {/* REMOVED: Status and Vendor columns */}
                                <th>Remarks</th>
                                {/* ADDED: New columns */}
                                <th>Movements</th>
                                ...
                                return (
                                    <React.Fragment key={uniqueId}>
                                        <tr className="main-row">
                                            ...
                                            {/* REMOVED: Status and Vendor cells */}
                                            <td className="remarks-cell">
                                                {report.Remarks || 'No remarks'}
                                            </td>
                                        ...
                                        {expandedRow === uniqueId && (
                                            <tr className="detail-row">
                                                <td colSpan={11}> {/* Updated colSpan from 10 to 11 */}
                                                    <div className="detail-panel">
                                                        <div className="detail-section">
                                                            <h4>PO Details</h4>
                                                            <div className="detail-grid">
                                                                ...
                                                                <div><strong>DM:</strong> {report.DM}</div>
                                                                <div><strong>Order Number:</strong> {report['Order Number']}</div>
                                                                {/* ADDED: Vendor and Status in expanded view */}
