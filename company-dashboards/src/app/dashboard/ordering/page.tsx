...
//     const [orderingData, setOrderingData] = useState<Row[]>([]);
...
//     // Enhanced search function for ordering data
//     const searchData = useCallback((data: OrderingReport[], searchTerm: string): OrderingReport[] => {
//         if (!searchTerm.trim()) {
//             return data;
//         }

//         const searchLower = searchTerm.toLowerCase().trim();
//         console.log(`🔍 Searching ordering data for: "${searchLower}" in ${data.length} records`);

//         const results = data.filter((report) => {
//             // Search across key ordering fields
...
//         console.log(`✅ Found ${results.length} ordering results for: "${searchLower}"`);
//         return results;
//     }, []);
...
//         if (searchTerm.trim()) {
//             return searchData(currentData, searchTerm);
//         }
//         return currentData;
//     }, [currentData, searchTerm, searchData]);
...
//                     orderCount: marketTotals.orderCount,
//                     rows: []
//                 };
//             });
...
//         return filterByDate(searchedData);
//     }, [searchedData, dateFilter]);
...
//         const detailedData = filteredByDateData; // This is the drilled-down data
...
//         const enhancedReports = paginatedDetailedData.map(report => {
...
//                                     onClick={() => {
//                                         const csvData = currentView === 'detailed' ? filteredByDateData : currentData;
//                                         if (csvData.length) {
...
//                         <input
//                             type="text"
//                             placeholder="Search by Market, Store, PO Number, Vendor..."
//                             value={searchTerm}
...
export default function OrderingReportsPage() {
    ...
    const exportToXLSX = useCallback(async () => {
        try {
            ...
            // Define all sheets to export
            const sheets = [
                { name: 'PO Listing', data: poListingData },
                { name: 'Receiving', data: receivingData },
                { name: 'Tracking', data: trackingData },
                { name: 'Disputes', data: disputesData },
                { name: 'Main', data: mainData },
                { name: 'Ordering', data: orderingData },
                { name: 'Current View', data: currentView === 'detailed' ? filteredByDateData : currentData }
            ];

            console.log('📊 Preparing sheets:', sheets.map(s => s.name));

            // Add each data source as a separate sheet
            sheets.forEach(({ name, data }) => {
                if (data && data.length > 0) {
                    ...
                    try {
                        ...
                        console.log(`✅ Successfully added sheet: ${name}`);
                    } catch (sheetError) {
                        console.error(`❌ Error adding sheet ${name}:`, sheetError);
                    }
                } else {
                    console.warn(`⚠️ No data for sheet: ${name}`);
                }
            });

            // Generate and download the file
            const fileName = `ordering_reports_${new Date().toISOString().split('T')[0]}.xlsx`;
    ...
    }, [poListingData, receivingData, trackingData, disputesData, mainData, orderingData, currentView, filteredByDateData, currentData]);
                ...
                <main className="main-area">
                    ...
                    {exportLoading && (
                        <div className="export-loading">
                            <div className="loading-spinner"></div>
                            <p>Preparing XLSX export with all sheets...</p>
                        </div>
                    )}
